...
; --- registros (solo si no existen ya) ---
IF !DEF(rDIV)
  DEF rDIV  EQU $FF04
ENDC
IF !DEF(rTIMA)
...
; --- bits/constantes HW timer ---
IF !DEF(IEF_TIMER)
  DEF IEF_TIMER  EQU %00000100
ENDC
DEF TACF_EN    EQU %00000100
DEF TACF_4KHZ  EQU %00000000
DEF TICKS_PER_SECOND   EQU 16          ; 4KHz / 256 -> 16Hz (con TMA=0)

; --- layout HUD (solo si no existen ya) ---
...
DEF TIMER_MAX_L        EQU $E7        ; 999 = $03E7
...
; Copia 10 tiles (0..9) a VRAM desde DIGIT_TILE_BASE
Timer_LoadTiles:
  ld hl, DigitsTiles
...
; Inicializa HW del timer: 4KHz, TIMA/TMA a 0, IE(TIMER)=1
Timer_Init:
  xor a
  ld [wTimerSubTicks], a
  ld [wTimerFlag1s], a
...
; HL = segundos (0..999). Satura y pre-calcula dígitos.
Timer_SetSecsHL:
  ld a, h
  cp TIMER_MAX_H+1
  jr c, .h_ok
...
; Convierte HL(segundos) a tiles (centenas, decenas, unidades)
Timer_ComputeDigits:
  ld a, [wTimerHi]
  ld h, a
  ld a, [wTimerLo]
...
; Inicializa HUD del timer (usa Update para pintar)
Timer_HUD_Init:
  jp Timer_HUD_Update

; Escribe H,T,O en la Window (en VBlank). Oculta ceros líderes.
Timer_HUD_Update:
  ld a, [wTileHundreds]
  cp DIGIT_TILE_BASE
  jr z, .checkTens
...
; ISR de Timer (1 tick = 1/16 s). Sube flag cada 1 s.
Timer_ISR:
  push af
  push hl
  ld hl, rIF
